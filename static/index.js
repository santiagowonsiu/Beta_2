...
//**
 /* FOR THE ON AND OFF BUTTON OF THE VIDEO_FEED
 */

 document.getElementById('start').addEventListener('click', function() {
    fetch('/views/video_feed', { method: 'GET' })
    .then(response => {
        if(response.ok) {
            console.log('Video feed started');
        } else {
            console.log('Error starting video feed');
        }
    })
...
document.getElementById('stop').addEventListener('click', function() {
    fetch('/views/stop_video_feed', { method: 'GET' })
    .then(response => {
        if(response.ok) {
            console.log('Video feed stopped');
        } else {
            console.log('Error stopping video feed');
        }
    })
    .catch(error => console.error('Error:', error));
});
